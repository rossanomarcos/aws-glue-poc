...
object DataCleaningLambda {
  def main(args: Array[String]): Unit = {
    ...
    val dbName = "payments"
    val tblName = "glue_sample_input_2"

    // S3 location for output
    val outputDir = "s3://glue-sample-target-2/output-dir/medicare_parquet"
    ...
    glueContext.getSinkWithFormat(connectionType = "s3",
                                options = JsonOptions(Map("path" -> outputDir)),
                                format = "parquet").writeDynamicFrame(medicareNest)
  }
}
